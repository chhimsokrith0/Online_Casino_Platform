...
const AllFishingGames: React.FC<AllGamesProps> = ({ locale }) => {
  const sectionRef = useRef<HTMLDivElement | null>(null);
  const router = useRouter();
  const initialCategory = "allGames";
  ...
  useEffect(() => {
    const fetchGames = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/fishing?category=${category}&search=${searchTerm}`);
...
export default AllFishingGames;
