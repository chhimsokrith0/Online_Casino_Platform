...
const AllLottoGames: React.FC<AllLiveCasinoGamesProps> = ({ locale }) => {
  const sectionRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const fetchGames = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/lotto?category=${category}&search=${searchTerm}`);
