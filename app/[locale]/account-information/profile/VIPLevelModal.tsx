...
import WalletModal from "@/components/Navbar/WalletModal/WalletModal"; // <-- Adjust import path if needed

interface UpgradeVipModalProps {
  onClose: () => void;
}

const UpgradeVipModal: React.FC<UpgradeVipModalProps> = ({ onClose }) => {
  const t = useTranslations("accountInformation");

  // Track which VIP level tab is active
  const [activeTab, setActiveTab] = useState("Bronze");
  // Control whether we show the WalletModal
  const [isWalletModalOpen, setIsWalletModalOpen] = useState(false);

  // Data for VIP levels and challenges
  const levels = [
    {
      ...
      challenges: [
        {
          task: t("profile.VIPLevelModal.challenges.list.task1.description"),
          amount: "1,000.00",
          action: "Deposit",
          progress: 50,
          isNew: true,
        ...
        {
          task: t("profile.VIPLevelModal.challenges.list.task2.description"),
          amount: "1,000.00",
          action: "Bet",
          progress: 30,
          isNew: false,
  ...
  // Find the current level’s data based on the active tab
  const activeLevel = levels.find((level) => level.name === activeTab);

  // Handlers to open/close the WalletModal
  ...
  return (
    <>
      {/* Main VIP modal backdrop & container */}
      <motion.div
        ...
        >
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="mb-6 sm:mb-8 text-center">
              ...
              >
                {t("profile.VIPLevelModal.upgradeTitle")}{" "}
                <span className="text-white">
                  {t("profile.VIPLevelModal.MemberLevel")}
                </span>
              </h3>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              <h4 className="text-yellow-500 font-bold mb-4">
                {activeTab} {t("profile.VIPLevelModal.challenges.title")}
              </h4>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-gray-400">
                  <thead className="bg-gray-700 text-gray-400">
                    <tr>
                      <th className="p-3 sm:p-4 text-xs sm:text-sm">
                        {t("profile.VIPLevelModal.challenges.th1")}
                      </th>
                      <th className="p-3 sm:p-4 text-xs sm:text-sm">
                        {t("profile.VIPLevelModal.challenges.th2")}
                      </th>
                      <th className="p-3 sm:p-4 text-xs sm:text-sm text-right">
                        {t("profile.VIPLevelModal.challenges.th3")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {activeLevel?.challenges.map((challenge, index) => (
                      ...
                      >
                        ...
                        <td className="p-3 sm:p-4 text-right">
                          {/* Different actions for "Deposit", "Bet", or others */}
                          {challenge.action === "Deposit" ? (
                            <motion.button
                              whileHover={{ scale: 1.1 }}
                              onClick={openWalletModal}
                              className="bg-yellow-500 text-black px-4 py-2 sm:px-6 sm:py-2 rounded-full shadow hover:bg-yellow-600 relative transition-all text-xs sm:text-sm"
                            >
                              {challenge.action}
                              {challenge.isNew && (
                                <span
                            ...
                            <Link href="/Games/slots" legacyBehavior>
                              <a>
                                <motion.button
                                  whileHover={{ scale: 1.1 }}
                                  className="bg-yellow-500 text-black px-4 py-2 sm:px-6 sm:py-2 rounded-full shadow hover:bg-yellow-600 relative transition-all text-xs sm:text-sm"
                            ...
                            <button
                              className="bg-gray-500 text-black px-4 py-2 sm:px-6 sm:py-2 rounded-full shadow cursor-not-allowed relative transition-all text-xs sm:text-sm"
                              disabled
                            >
                              {challenge.action}
                            </button>
      ...
      {/* Wallet Modal rendered conditionally */}
      {isWalletModalOpen && (
        <WalletModal
